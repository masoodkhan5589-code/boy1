...
class IVASmsService(IOtpService):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.range_number: Optional[str] = None
    ...
    def create(self, max_attempts: int = 5, **kwargs) -> Optional[OTPServiceResponse]:
        try:
            token = random.choice(self.access_token.split('|'))
            url = f'{self.api_url}/ivasms/services/create/?selected_countries={token}'
            response_data = self._send_request("get", url, verify_ssl=False)

            if response_data and response_data.get("status") == "success":
                contact = str(response_data.get("data", {}).get("phone", None))
                return OTPServiceResponse(id=contact, contact=contact, additional_value=token)
    ...
    def get_code(self, **kwargs) -> Union[str, None]:
        ...
        phone_number = kwargs.get('email_address', None)
        if not phone_number or not self.range_number:
            return None

        while not code and time.time() < start_time + self.time_out:
            try:
                url = f'{self.api_url}/ivasms/services/sms/?number={phone_number}&phone_range={self.range_number}'
                body_response = self._send_request("get", url, verify_ssl=False)

                response_code = body_response.get("data", {}).get('otp', None)
                print(body_response)

                if body_response and response_code:
                    log_message = f"{phone_number}|{response_code}"
